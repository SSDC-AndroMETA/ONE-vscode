...
export class Metadata {
  ...
  public static async set(hash: string, obj: object) {
    ...
    const metadataUri = vscode.Uri.joinPath(
      ...
      `.meta/hash_objects/${hash.substring(0, 2)}/${hash.substring(2)}.json`);
    await vscode.workspace.fs.writeFile(metadataUri, Buffer.from(JSON.stringify(obj, null, 4), 'utf8'));
  }
  ...
  public static async disable(input: {[key: string]: any}) {
    ...
    if (metadata === undefined || !metadata[relPath] || Object.keys(metadata[relPath]).length !== 0) {
      return;
    }
  ...
  //   }
  //   await vscode.workspace.fs.writeFile(metadataUri, Buffer.from(JSON.stringify(metadata, null, 4), 'utf8'));
  // }
}
