...
import {obtainWorkspaceRoot,isOneExploereTargetFile} from '../Utils/Helpers';
...
export class MetadataEventManager {
  ...
  public static register(context: vscode.ExtensionContext) {
    ...
    let registrations = [
      ...
      manager.fileWatcher.onDidCreate(async uri => {
        ...
        } else if (isOneExploereTargetFile(uri)) {
          if (caseFlag) {
            ////console.log('b');
  ...
  async createDirEvent(input: {[key: string]: any}) {
    ...
    for (let uri of uriList) {
      if (isOneExploereTargetFile(uri)) {
  ...
  public static async moveFileEvent(input: {[key: string]: any}) {
    ...
    //console.log('1');
    const pathToHash = await PathToHash.getInstance();
    if (isOneExploereTargetFile(fromUri) && !isOneExploereTargetFile(toUri)) {
      // when the file is renamed from a valid file name to a invalid file name
      // ex. a.log > a.txt
      await MetadataEventManager.deleteFileEvent(fromUri);
      //console.log('2');
    } else if (!isOneExploereTargetFile(fromUri) || !isOneExploereTargetFile(toUri)) {
      //console.log('3');
      return;
    }
